...
/*   Updated: 2024/12/23 14:05:44 by donghwi2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (infile == true && io->infile)//입력파일이면
	{
		if (io->fd_in == -1 || (io->outfile && io->fd_out == -1))
			return (false);//이전파일 열기 실패 혹은 출력파일 열기 실패시 false
	...
	else if (infile == false && io->outfile)//출력파일이면
	{
...
{
	if (!remove_old_file_ref(io, true))//이전 파일 참조 삭제 등 정리
		return ;
	io->infile = ft_strdup(file);//새 파일명 저장
...
void	parse_input(t_command **last_cmd, t_token **token_lst)
{
	...
	temp = *token_lst;
	cmd = lst_last_cmd(*last_cmd);//가장 최근 cmd노드로 이동
	init_io(cmd);//io 구조체 초기화
	open_infile(cmd->io_fds, temp->next->str, temp->next->str_backup);//입력파일 열기 시도
	if (temp->next->next)
		temp = temp->next->next;
	else
		temp = temp->next;
